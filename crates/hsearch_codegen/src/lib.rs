...
use util::{collect_bits, preserved_bits};
...
fn stage1_ridge_orientation(r: Mat4, v: Vec4, o: u8) -> u8 {
    const ORDER: [hsearch_core::Axis; 4] = [W, X, Y, Z];
    match o {
        0b00 | 0b11 => o,
        0b10 | 0b01 => {
            let old_axis = v.unwrap_first_nonzero_axis(ORDER);
            let new_axis = (r * v).unwrap_first_nonzero_axis(ORDER);
...
fn stage1() -> String {
    let target_e: u32 = collect_bits(edges().map(|v| v[W] == 0 && v[Z] <= 0));
    let target_r: u64 = collect_bits(
        ridges()
            .map(|v| v[W] == 0 && v[Z] <= 0)
            .flat_map(|b| [b, true]),
    );
    ...
    let ro =
        OrientationLut::new(ridges(), 4, stage1_ridge_orientation).to_rust_code(64, 0, 2, "r_op");
...
fn stage2() -> String {
    ...
    let blocks: [(&Group, fn(Vec4) -> bool); 3] = [
    ...
    for (i, (symmetry, predicate)) in blocks.into_iter().enumerate() {
        writeln!(out, "    pub const TARGET{}: &[Self] = &[", i + 1).unwrap();
        let mut values = symmetry
            .elems()
            .into_iter()
            .map(|m| {
                (
                    collect_bits::<u16>(s2_ridges().map(|v| predicate(m * v))),
                    collect_bits::<u64>(s2_edges().flat_map(|v| [predicate(m * v); 2])),
                    collect_bits::<u32>(s2_corners().flat_map(|v| [predicate(m * v); 2])),
                )
            })
            .sorted()
            .dedup();
        for (r, e, c) in values.by_ref() {
            writeln!(out, "        Self::new(0x{r:04x}, 0x{e:016x}, 0x{c:08x}),").unwrap();
        }
        out.push_str("    ];\n");
    }
    let r = PermutationLut::new(s2_ridges()).to_rust_code(16, 0, 1, "r_p");
    let eo = OrientationLut::new(s2_edges(), 4, |r, v, o| match o {
        0 => o,
        _ => {
    ...
    writeln!(out, "        let r_p = {r};").unwrap();
    writeln!(out, "        let e_op = {eo};").unwrap();
    writeln!(out, "        let e_op = {ep};").unwrap();
    writeln!(out, "        let c_o = {co};").unwrap();
    writeln!(out, "        let c_o = {cp};").unwrap();
    out.push_str("        Self { r_p, e_op, c_o }\n    }\n}\n");
    out
}

fn stage3_ridges() -> impl Iterator<Item = Vec4> {
    ridges().filter(|v| v[W] == 1 || v[Z] == 1)
}
fn stage3_edges() -> impl Iterator<Item = Vec4> {
    edges().filter(|v| v[W] == 1 || v[Z] == 1)
}
fn stage3_corners() -> impl Iterator<Item = Vec4> {
    corners().filter(|v| v[W] == 1 || v[Z] == 1)
}

fn stage3() -> String {
    let solved_r: u16 = collect_bits(stage3_ridges().map(|v| v[W] != 0));
    let solved_e: u64 = collect_bits(std::iter::chain(
        stage3_edges().flat_map(|v| [v[W] != 0, false]),
        stage3_corners().flat_map(|_| [true, true]),
    ));
    let symmetry = Group::new(vec![Mat4::rot(X, Y)]);
    let blocks: [fn(Vec4) -> bool; 2] = [
        |v| v[X] <= 0 && v[Y] <= 0 && v[Z] <= 0 && v[W] > 0,
        |v| v[Y] <= 0 && v[Z] <= 0 && v[W] > 0,
    ];
    let mut out = format!(
        "impl Stage3 {{\n    pub const SOLVED: Self = Self {{ r_p: 0x{solved_r:04x}, e_op_c_o: 0x{solved_e:016x} }};\n"
    );
    for (i, predicate) in blocks.into_iter().enumerate() {
        writeln!(out, "    pub const TARGET{}: &[Self] = &[", i + 1).unwrap();
        for (r, e) in symmetry
            .elems()
            .into_iter()
            .map(|m| {
                (
                    collect_bits::<u16>(stage3_ridges().map(|v| predicate(m * v))),
                    collect_bits::<u64>(std::iter::chain(
                        stage3_edges().flat_map(|v| [predicate(m * v); 2]),
                        stage3_corners().flat_map(|v| [predicate(m * v); 2]),
                    )),
                )
            })
            .sorted()
            .dedup()
        {
            writeln!(out, "        Self::new(0x{r:04x}, 0x{e:016x}),").unwrap();
        }
        out.push_str("    ];\n");
    }
    let r = PermutationLut::new(stage3_ridges()).to_rust_code(16, 0, 1, "r_p");
    let eo = OrientationLut::new(stage3_edges().chain(stage3_corners()), 4, |r, v, o| {
        if v.taxicab_norm() == 3 {
            match o {
                0 => o,
                _ => {
    ...
    let ep = PermutationLut::new(stage3_edges().chain(stage3_corners()))
        .to_rust_code(64, 0, 2, "e_op_c_o");
    ...
    writeln!(out, "        let r_p = {r};").unwrap();
    writeln!(out, "        let e_op_c_o = {eo};").unwrap();
    writeln!(out, "        let e_op_c_o = {ep};").unwrap();
    out.push_str("        Self { r_p, e_op_c_o }\n    }\n}\n");
    out
}
...
fn stage4() -> String {
    ...
    match_src.push_str("            _ => XyRot::IDENT,\n        }\n    }\n");
    ...
    out.push_str("}\n\nimpl Stage4 {\n");
    out.push_str(&match_src);
    out.push_str("}\n\nimpl Stage4 {\n    fn generated_do_twist(self, twist: Twist) -> Self {\n        let Self { bits } = self;\n");
    writeln!(out, "        let bits = {o};").unwrap();
    writeln!(out, "        let bits = {p2};").unwrap();
    writeln!(out, "        let bits = {r};").unwrap();
    out.push_str("        Self { bits }\n    }\n}\n");
    writeln!(
        out,
...
fn v2() -> String {
    let solved = format!(
        "V2Stage1 {{ r: 0x{:012x}, e: 0x{:024x}, c: 0x{:016x} }}",
    ...
    let mut out = format!("impl V2Stage1 {{\n    pub const SOLVED: Self = {solved};\n");
    ...
    out.push_str("}\n\nimpl V2Stage1 {\n    fn generated_do_twist(self, twist: Twist) -> Self {\n        let Self { r, e, c } = self;\n");
    writeln!(out, "        let r = {r};").unwrap();
    writeln!(out, "        let e = {e};").unwrap();
    writeln!(out, "        let c = {c};").unwrap();
    out.push_str("        Self { r, e, c }\n    }\n}\n");
    out
}
